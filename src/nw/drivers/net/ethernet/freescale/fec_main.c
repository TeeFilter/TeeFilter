...
#include "fec.h"
#include "fec_helpers.h"

#define TX_BD_IN_CACHABLE_MEMORY
#define RX_BD_IN_CACHABLE_MEMORY

#define TX_DATA_WITHOUT_CACHE_MAINT
#define RX_DATA_WITHOUT_CACHE_MAINT


#define my_readl_poll_timeout_atomic(addr, val, cond, delay_us, timeout_us) \
	readx_poll_timeout_atomic(my_readl, addr, val, cond, delay_us, timeout_us)
			  ...
			  struct sk_buff *skb, unsigned index)
{
	 ...
	 */
	#ifndef TX_BD_IN_CACHABLE_MEMORY
	dma_wmb();
	#endif
	...
	// Inform the service that there was an update to the ring buffer
	tx_tos(txq->bd.qid, fec_enet_get_bd_index(txq->bd.cur, &txq->bd));

	/* Trigger transmission start */
	my_writel(0, txq->bd.reg_desc_active);
	return 0;
}
...
{
	while (bdp != end) {
		#ifdef TX_DATA_WITHOUT_CACHE_MAINT
		;
		#else
				   ...
				   struct sk_buff *skb, struct net_device *ndev)
{
	...
	if (entries_free < MAX_SKB_FRAGS + 1) {
		printk(KERN_ALERT "ERROR, almost full\n");
				   ...
				   struct net_device *ndev)
{
	...
	if (tso_count_descs(skb) >= fec_enet_get_free_txdesc_num(txq)) {
		printk(KERN_ALERT "We do not have enough BDs!\n");
	...
	if (fec_enet_clear_csum(skb, ndev)) {
		dev_kfree_skb_any(skb);
		printk(KERN_ALERT "We are in the clear CSUM check!\n");
		return NETDEV_TX_OK;
	}
...
static void fec_enet_active_rxring(struct net_device *ndev)
{
	...
	for (i = 0; i < fep->num_rx_queues; i++)
		my_writel(0, fep->rx_queue[i]->bd.reg_desc_active);
}
...
static void fec_enet_enable_ring(struct net_device *ndev)
{
	...
	for (i = 0; i < fep->num_rx_queues; i++) {
		rxq = fep->rx_queue[i];
		my_writel(rxq->bd.dma, fep->hwp + FEC_R_DES_START(i));
		my_writel(PKT_MAXBUF_SIZE, fep->hwp + FEC_R_BUFF_SIZE(i));

		/* enable DMA1/2 */
		if (i)
			my_writel(RCMR_MATCHEN | RCMR_CMP(i),
			       fep->hwp + FEC_RCMR(i));
	}

	for (i = 0; i < fep->num_tx_queues; i++) {
		txq = fep->tx_queue[i];
		my_writel(txq->bd.dma, fep->hwp + FEC_X_DES_START(i));

		/* enable DMA1/2 */
		if (i)
			my_writel(DMA_CLASS_EN | IDLE_SLOPE(i),
...
fec_restart(struct net_device *ndev)
{
	 ...
	 */
	if (fep->quirks & FEC_QUIRK_HAS_AVB) {
		my_writel(0, fep->hwp + FEC_ECNTRL);
	} else {
		my_writel(1, fep->hwp + FEC_ECNTRL);
		udelay(10);
	}
	...
	my_writel((__force u32)cpu_to_be32(temp_mac[0]),
	       fep->hwp + FEC_ADDR_LOW);
	my_writel((__force u32)cpu_to_be32(temp_mac[1]),
	       fep->hwp + FEC_ADDR_HIGH);

	/* Clear any outstanding interrupt, except MDIO. */
	my_writel((0xffffffff & ~FEC_ENET_MII), fep->hwp + FEC_IEVENT);
	...
	if (fep->full_duplex == DUPLEX_FULL) {
		/* FD enable */
		my_writel(0x04, fep->hwp + FEC_X_CNTRL);
	} else {
		/* No Rcv on Xmit */
		rcntl |= 0x02;
		my_writel(0x0, fep->hwp + FEC_X_CNTRL);
	}

	/* Set MII speed */
	my_writel(fep->phy_speed, fep->hwp + FEC_MII_SPEED);

#if !defined(CONFIG_M5272)
	if (fep->quirks & FEC_QUIRK_HAS_RACC) {
		val = my_readl(fep->hwp + FEC_RACC);
		/* align IP header */
		...
		if (fep->csum_flags & FLAG_RX_CSUM_ENABLED)
			...
			val &= ~FEC_RACC_OPTIONS;
		my_writel(val, fep->hwp + FEC_RACC);
		my_writel(PKT_MAXBUF_SIZE, fep->hwp + FEC_FTRL);
	}
#endif
	...
	if (fep->quirks & FEC_QUIRK_ENET_MAC) {
		...
		if (fep->quirks & FEC_QUIRK_USE_GASKET) {
			...
			my_writel(0, fep->hwp + FEC_MIIGSK_ENR);
			while (my_readl(fep->hwp + FEC_MIIGSK_ENR) & 4)
				udelay(1);
			...
			my_writel(cfgr, fep->hwp + FEC_MIIGSK_CFGR);

			/* re-enable the gasket */
			my_writel(2, fep->hwp + FEC_MIIGSK_ENR);
		}
#endif
	}
	...
	if ((fep->pause_flag & FEC_PAUSE_FLAG_ENABLE) ||
		...
		my_writel(FEC_ENET_RSEM_V, fep->hwp + FEC_R_FIFO_RSEM);
		my_writel(FEC_ENET_RSFL_V, fep->hwp + FEC_R_FIFO_RSFL);
		my_writel(FEC_ENET_RAEM_V, fep->hwp + FEC_R_FIFO_RAEM);
		my_writel(FEC_ENET_RAFL_V, fep->hwp + FEC_R_FIFO_RAFL);

		/* OPD */
		my_writel(FEC_ENET_OPD_V, fep->hwp + FEC_OPD);
	} else {
		rcntl &= ~FEC_ENET_FCE;
	}
#endif /* !defined(CONFIG_M5272) */

	my_writel(rcntl, fep->hwp + FEC_R_CNTRL);

	/* Setup multicast filter. */
	set_multicast_list(ndev);
#ifndef CONFIG_M5272
	my_writel(0, fep->hwp + FEC_HASH_TABLE_HIGH);
	my_writel(0, fep->hwp + FEC_HASH_TABLE_LOW);
#endif
	...
	if (fep->quirks & FEC_QUIRK_ENET_MAC) {
		...
		my_writel(1 << 8, fep->hwp + FEC_X_WMRK);
	}
	...
	my_writel(0 << 31, fep->hwp + FEC_MIB_CTRLSTAT);
#endif

	/* And last, enable the transmit and receive processing */
	my_writel(ecntl, fep->hwp + FEC_ECNTRL);
	...
	if (fep->link)
		my_writel(FEC_DEFAULT_IMASK, fep->hwp + FEC_IMASK);
	else
		my_writel(0, fep->hwp + FEC_IMASK);
...
{
	struct fec_enet_private *fep = netdev_priv(ndev);
	u32 rmii_mode = my_readl(fep->hwp + FEC_R_CNTRL) & (1 << 8);
	u32 val;

	/* We cannot expect a graceful transmit stop without link !!! */
	if (fep->link) {
		my_writel(1, fep->hwp + FEC_X_CNTRL); /* Graceful transmit stop */
		udelay(10);
		if (!(my_readl(fep->hwp + FEC_IEVENT) & FEC_ENET_GRA))
	...
	if (!(fep->wol_flag & FEC_WOL_FLAG_SLEEP_ON)) {
		...
		my_writel(FEC_DEFAULT_IMASK | FEC_ENET_WAKEUP, fep->hwp + FEC_IMASK);
		val = my_readl(fep->hwp + FEC_ECNTRL);
		val |= (FEC_ECR_MAGICEN | FEC_ECR_SLEEP);
		my_writel(val, fep->hwp + FEC_ECNTRL);
	}
	my_writel(fep->phy_speed, fep->hwp + FEC_MII_SPEED);
	...
	if (fep->quirks & FEC_QUIRK_ENET_MAC &&
		...
		my_writel(2, fep->hwp + FEC_ECNTRL);
		my_writel(rmii_mode, fep->hwp + FEC_R_CNTRL);
	}
}
...
static void fec_txq(struct net_device *ndev, struct fec_enet_priv_tx_q *txq)
{
	...
	// Inform the service that we are about the read the ring buffer
	tx_frs(txq->bd.qid, fec_enet_get_bd_index(bdp, &txq->bd));
	...
	while (bdp != READ_ONCE(txq->bd.cur)) {
		...
		if (status & BD_ENET_TX_READY) {
			if (!my_readl(txq->bd.reg_desc_active)) {
		...
		if (status & (BD_ENET_TX_HB | BD_ENET_TX_LC |
			...
			if (status & BD_ENET_TX_HB)  /* No heartbeat */ {
				printk(KERN_ALERT "No heartbeart\n");
				ndev->stats.tx_heartbeat_errors++;
			} if (status & BD_ENET_TX_LC)  /* Late collision */ {
				printk(KERN_ALERT "Late collision\n");
				ndev->stats.tx_window_errors++;
			} if (status & BD_ENET_TX_RL)  /* Retrans limit */ {
				printk(KERN_ALERT "Retrans limit\n");
				ndev->stats.tx_aborted_errors++;
			} if (status & BD_ENET_TX_UN)  /* Underrun */ {
				printk(KERN_ALERT "Underrun\n");
				ndev->stats.tx_fifo_errors++;
			} if (status & BD_ENET_TX_CSL) /* Carrier lost */ {
				printk(KERN_ALERT "Carrier lost\n");
...
fec_enet_new_rxbdp(struct net_device *ndev, struct bufdesc *bdp, struct sk_buff *skb)
{
	...
	#ifdef RX_DATA_WITHOUT_CACHE_MAINT
	dmabuf = virt_to_phys(skb->data);
	#else
			       ...
			       struct bufdesc *bdp, u32 length, bool swap)
{
	...
	#ifndef RX_DATA_WITHOUT_CACHE_MAINT
		   ...
		   int budget)
{
	...
	// Inform the service that we are about the read the ring buffer
	rx_frs(rxq->bd.qid, fec_enet_get_bd_index(bdp, &rxq->bd), budget);
...
rx_processing_done:
		...
		// We tell the secure world that we have emptied a buffer as soon as we
		// actually have done it. This is rather agressive.
		// rx_tos(rxq->bd.qid, fec_enet_get_bd_index(bdp, &rxq->bd));

		/* Doing this here will keep the FEC running while we process
		 * incoming frames.  On a heavily loaded network, we should be
		 * able to keep up at the expense of system resources.
		 */
		my_writel(0, rxq->bd.reg_desc_active);
	...
	// take not updated cur and tell how many packets where received
	rx_tos(rxq->bd.qid, fec_enet_get_bd_index(rxq->bd.cur, &rxq->bd), pkt_received);
...
fec_enet_interrupt(int irq, void *dev_id)
{
	...
	uint eir = my_readl(fep->hwp + FEC_IEVENT);
	uint int_events = eir & my_readl(fep->hwp + FEC_IMASK);
	...
	if (int_events) {
		ret = IRQ_HANDLED;
		my_writel(int_events, fep->hwp + FEC_IEVENT);
	}
	return ret;
}
...
static int fec_enet_napi_q3(struct napi_struct *napi, int budget)
{
	...
	do {
		events = my_readl(fep->hwp + FEC_IEVENT);
		if (fep->events) {
			events |= fep->events;
			fep->events = 0;
		}
		...
		my_writel(events, fep->hwp + FEC_IEVENT);
...
static int fec_get_mac(struct net_device *ndev)
{
	...
	if (!is_valid_ether_addr(iap)) {
		*((__be32 *)&tmpaddr[0]) =
			cpu_to_be32(my_readl(fep->hwp + FEC_ADDR_LOW));
		*((__be16 *)&tmpaddr[4]) =
			cpu_to_be16(my_readl(fep->hwp + FEC_ADDR_HIGH) >> 16);
		iap = &tmpaddr[0];
	}
...
static int fec_enet_mdio_wait(struct fec_enet_private *fep)
{
	...
	ret = my_readl_poll_timeout_atomic(fep->hwp + FEC_IEVENT, ievent,
					ievent & FEC_ENET_MII, 15, 30000000000);

	if (!ret)
		my_writel(FEC_ENET_MII, fep->hwp + FEC_IEVENT);
...
static int fec_enet_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
	...
	if (is_c45) {
		...
		my_writel(frame_start | FEC_MMFR_OP_ADDR_WRITE |
	...
	/* start a read op */
	my_writel(frame_start | frame_op |
		FEC_MMFR_PA(mii_id) | FEC_MMFR_RA(frame_addr) |
	...
	ret = FEC_MMFR_DATA(my_readl(fep->hwp + FEC_MII_DATA));
...
static int fec_restore_mii_bus(struct net_device *ndev)
{
	...
	my_writel(0xffc00000, fep->hwp + FEC_IEVENT);
	my_writel(fep->phy_speed, fep->hwp + FEC_MII_SPEED);
	my_writel(FEC_ENET_MII, fep->hwp + FEC_IMASK);
	my_writel(FEC_ENET_ETHEREN, fep->hwp + FEC_ECNTRL);
...
static int fec_enet_mii_init(struct platform_device *pdev)
{
	...
	if (fep->quirks & FEC_QUIRK_CLEAR_SETUP_MII) {
		 ...
		 *	- mscr[7:0]_not_zero
		 */
		my_writel(0, fep->hwp + FEC_MII_DATA);
	}
...
static void fec_enet_update_ethtool_stats(struct net_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(fec_stats); i++)
		fep->ethtool_stats[i] = my_readl(fep->hwp + fec_stats[i].offset);
}
...
static void fec_enet_clear_ethtool_stats(struct net_device *dev)
{
	...
	my_writel(FEC_MIB_CTRLSTAT_DISABLE, fep->hwp + FEC_MIB_CTRLSTAT);

	for (i = 0; i < ARRAY_SIZE(fec_stats); i++)
		my_writel(0, fep->hwp + fec_stats[i].offset);

	/* Don't disable MIB statistics counters */
	my_writel(0, fep->hwp + FEC_MIB_CTRLSTAT);
}
...
static void fec_enet_itr_coal_set(struct net_device *ndev)
{
	...
	my_writel(tx_itr, fep->hwp + FEC_TXIC0);
	my_writel(rx_itr, fep->hwp + FEC_RXIC0);
	if (fep->quirks & FEC_QUIRK_HAS_AVB) {
		my_writel(tx_itr, fep->hwp + FEC_TXIC1);
		my_writel(rx_itr, fep->hwp + FEC_RXIC1);
		my_writel(tx_itr, fep->hwp + FEC_TXIC2);
		my_writel(rx_itr, fep->hwp + FEC_RXIC2);
	}
}
...
static int fec_enet_eee_mode_set(struct net_device *ndev, bool enable)
{
	...
	my_writel(sleep_cycle, fep->hwp + FEC_LPI_SLEEP);
	my_writel(wake_cycle, fep->hwp + FEC_LPI_WAKE);
...
static int fec_enet_alloc_queue(struct net_device *ndev)
{
	...
	for (i = 0; i < fep->num_tx_queues; i++) {
		...
		#ifdef TX_BD_IN_CACHABLE_MEMORY
		txq->tso_hdrs = kmalloc(txq->bd.ring_size * TSO_HEADER_SIZE, GFP_KERNEL);
		txq->tso_hdrs_dma = virt_to_phys(txq->tso_hdrs);
		#else
...
static void set_multicast_list(struct net_device *ndev)
{
	...
	if (ndev->flags & IFF_PROMISC) {
		tmp = my_readl(fep->hwp + FEC_R_CNTRL);
		tmp |= 0x8;
		my_writel(tmp, fep->hwp + FEC_R_CNTRL);
		return;
	}
	...
	if (ndev->flags & IFF_ALLMULTI) {
		 ...
		 * filter to all 1's
		 */
		my_writel(0xffffffff, fep->hwp + FEC_GRP_HASH_TABLE_HIGH);
		my_writel(0xffffffff, fep->hwp + FEC_GRP_HASH_TABLE_LOW);
	...
	my_writel(hash_high, fep->hwp + FEC_GRP_HASH_TABLE_HIGH);
	my_writel(hash_low, fep->hwp + FEC_GRP_HASH_TABLE_LOW);
}
...
fec_set_mac_address(struct net_device *ndev, void *p)
{
	...
	my_writel(ndev->dev_addr[3] | (ndev->dev_addr[2] << 8) |
		(ndev->dev_addr[1] << 16) | (ndev->dev_addr[0] << 24),
		fep->hwp + FEC_ADDR_LOW);
	my_writel((ndev->dev_addr[5] << 16) | (ndev->dev_addr[4] << 24),
		fep->hwp + FEC_ADDR_HIGH);
	return 0;
}
...
static int fec_enet_init(struct net_device *ndev)
{
	...
	#if defined(TX_BD_IN_CACHABLE_MEMORY) && !defined(RX_BD_IN_CACHABLE_MEMORY)
	cbd_base = dmam_alloc_coherent(&fep->pdev->dev, fep->total_rx_ring_size * dsize, &bd_dma,
				       GFP_KERNEL);
	if (!cbd_base) {
		ret = -ENOMEM;
		goto free_queue_mem;
	...
	#elif defined(TX_BD_IN_CACHABLE_MEMORY) && defined(RX_BD_IN_CACHABLE_MEMORY)

	cbd_base = kmalloc(fep->total_rx_ring_size * dsize, GFP_KERNEL);
	if (!cbd_base) {
		ret = -ENOMEM;
		goto free_queue_mem;
	}
	bd_dma = virt_to_phys(cbd_base);
	...
	#ifdef TX_BD_IN_CACHABLE_MEMORY
	cbd_base = kmalloc(fep->total_tx_ring_size * dsize, GFP_KERNEL);
	if (!cbd_base) {
		ret = -ENOMEM;
		goto free_queue_mem;
	}
	...
	my_writel(FEC_RX_DISABLED_IMASK, fep->hwp + FEC_IMASK);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%08x %08x %08x %d %d %d\n", fep->events,
			my_readl(fep->hwp + FEC_IEVENT),
			my_readl(fep->hwp + FEC_IMASK),
